			...
			if (Number(value) === 0) {
				...
				['setQualityFor']('video', value)
					.then(() => {
						...
						that.player._setVideoQuality(value);
						this.player.videoQuality = value;
						that.player.controller.updateQuality({
							acceptQuality: that.player.videoData.acceptQuality,
							bp: that.player.videoData.bp,
							hasPaid: that.player.videoData.hasPaid,
							isPreview: that.player.videoData.isPreview,
						}, value);
						callback?.(true);
					})
					.catch((result: any) => {
