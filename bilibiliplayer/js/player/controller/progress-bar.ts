...
import SkipSegments from "./skip-segments";
...
export class ProgressBar {
	...
	private onTime() {
		...
		if (player.video) {
			...
			// 互动视频特殊处理（不走以下逻辑
			this.skipSegments?.autoSkipSegments(currentTime);
		}
	}
	newSkip(skipSegments: ISkipSegments) {
		// 切换视频后可跳过片段数据会变化，需要重建 SkipSegments 实例以应用新数据
		if (this.skipSegments) {
			this.skipSegments.reload(skipSegments);
		} else {
