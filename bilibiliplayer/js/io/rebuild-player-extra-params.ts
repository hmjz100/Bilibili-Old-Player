...
export interface ISkipSegments {
	hasSkip: boolean; // 是否在设置面板中显示跳过选项
	hasData: boolean; // 是否有可跳过的片段数据
	first: boolean;
	head?: number[];  // 片头时间段 [start, end]
	tail?: number[];  // 片尾时间段 [start, end]
}
...
function rebuildPlayerExtraParams(player: Player): IPlayerExtraParams | null | any {
	if (typeof player.window['getPlayerExtraParams'] === 'function' && player.config.type !== ContentType.Pugv) {
		...
		if (params) {
			...
			let skipSegments: ISkipSegments | null = null;
			if (params.headTail) {
				skipSegments = {
					hasData: false,
				...
				if (Array.isArray(op) && (op[0] || op[1])) {
					skipSegments.head = op;
				}
				if (Array.isArray(ed) && (ed[0] || ed[1])) {
					skipSegments.tail = ed;
				}
				if (skipSegments.head || skipSegments.tail) {
					skipSegments.first = params.headTail.first;
					skipSegments.hasData = true;
					skipSegments.hasSkip = true;
				}
			}
